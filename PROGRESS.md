...
| 2026-07-16 | **Item 0.5 deviations.** (a) Req 3-010 access reviews deferred to 0.11 — priority *Should*, no gating acceptance test; note V1-SCOPE does list "access review" under v1 Identity, so this is a real scope cut to revisit before v1 ships. (b) Reqs 3-001/3-002 SSO/MFA land as **claim validation only** (`acr`/`amr`), IdP pluggable, since no IdP is chosen. (c) Reqs 3-006/3-007 (external assurer, supplier access) deferred per V1-SCOPE. | ⚠ **Pending review** | — |
