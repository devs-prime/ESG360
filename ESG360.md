...
1. Create cycle and scope.
2. Generate tasks and notify owners.
3. Collect through UI, file, API or integration.
4. Run validations and anomaly detection.
5. Review, resolve comments and approve.
6. Close period and create assurance snapshot.
...
1. Register sources and boundaries.
2. Collect activity and contractual data.
3. Resolve factors and calculate gas-level emissions.
4. Validate completeness and outliers.
5. Consolidate by entity, scope and category.
6. Approve inventory and lock baseline.
...
1. Select framework and reporting perimeter.
2. Run applicability/materiality assessment.
3. Populate mapped quantitative datapoints.
4. Draft qualitative disclosures.
5. Review legal/finance/ESG consistency.
6. Sign off and publish immutable package.
...
1. Resolve tenant, boundary, period, metric/factor/formula versions and applicable hierarchy.
2. Load approved activity records and dimensions at unrounded precision.
3. Validate unit compatibility and convert to canonical unit.
4. Resolve factor according to configured hierarchy and validity.
5. Calculate gas-level emissions or metric result using decimal arithmetic.
6. Apply GWP set where CO2e is required.
7. Aggregate according to metric rules and consolidation boundary.
8. Create result records and lineage edges to every input, factor and formula version.
9. Run cross-checks, anomaly rules and reconciliation.
10. Publish CalculationCompleted event and retain execution manifest/hash.

## Calculation Pseudocode

result = SUM(activity.normalised\_quantity × factor.value × allocation\_ratio × conversion\_multiplier)
co2e = SUM(gas\_mass × GWP[gas, version])

# 39. Database Design Rules

* All business tables include tenant\_id, created\_at, created\_by, updated\_at, updated\_by and row\_version.
...
1. Source control with protected branches, signed commits/tags and peer review.
2. CI runs formatting, unit tests, SAST, dependency and licence checks.
3. Build produces signed SBOM and immutable container/package artifacts.
4. Automated integration, API contract, migration and security tests run in ephemeral environment.
5. Infrastructure-as-code changes use policy-as-code and approval.
6. Release deploys progressively using feature flags/canary or blue-green pattern.
7. Database migrations are backward compatible and rehearsed against representative volumes.
8. Post-deploy smoke tests and observability gates determine promotion or rollback.
9. Customer-impacting changes are documented with release notes and deprecation notices.
...
1. Discover source systems, historical periods, evidence, dimensions and data quality.
2. Configure tenant hierarchy, units, calendars, metric catalogue and frameworks.
3. Load master data and validate referential integrity.
4. Load historical activities/submissions with source IDs and evidence links.
5. Recalculate selected periods and reconcile against approved legacy reports.
6. Obtain data-owner and finance/ESG sign-off.
7. Freeze migration package and retain reconciliation evidence.
8. Activate production integrations and reporting cycle.
